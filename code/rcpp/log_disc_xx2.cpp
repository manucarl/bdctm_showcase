...
// [[Rcpp::export]]
mat A_matvecprod_elwise(const arma::mat & X, const arma::vec & y){
  int k = X.n_cols ;
...
vec gradf_logit(const arma::vec &params, const Rcpp::List  &xx){
  ...
  // mat Sb = A_matvecprod_elwise(S, params);
  // mat Sb = cheapHadamard(S, params).t();
  vec ppd = Fh - Fhl;
...
vec gradf_logitd3(const arma::vec &params, const Rcpp::List  &xx){
  ...
  vec v1 = X*bt;
  vec v2 = Xl*bt;
  v2(zero_ind).fill(-1000000);
  v1(max_ind).fill(1000000);
  vec Fh = plogis(v1);
  vec Fhl = plogis(v2);
