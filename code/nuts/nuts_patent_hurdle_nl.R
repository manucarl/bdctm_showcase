...
NUTS <- function(n_iter, xx, f, gr, ll, start, warmup = floor(n_iter/2),thin=1, burnin = floor(n_iter/2),
  ...
  tau2_out <- matrix(1, nrow=n_iter, ncol=8)
  ...
  for(iter in 1:n_iter){
    ...
    par <- beta_out[iter, 1:8]
    
    
    # # # 
    tau2_out[iter,1] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*ranks[[1]], hyper_b + as.matrix(0.5*t(par)%*%Klist[[1]]%*%par))
    Ks[[1]] <-Klist[[1]]/tau2
    # 
    
    
    par <- beta_out[iter, 9:19]  
    tau2_out[iter,2] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*ranks[[2]], hyper_b + as.matrix(0.5*t(par)%*%Klist[[2]]%*%par))
    Ks[[2]] <-Klist[[2]]/tau2
    
    par <- beta_out[iter, 20:30]  
    tau2_out[iter,3] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*ranks[[3]], hyper_b + as.matrix(0.5*t(par)%*%Klist[[3]]%*%par))
    Ks[[3]] <-Klist[[3]]/tau2
    
    par <- beta_out[iter, 31:41]  
    tau2_out[iter,4] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*ranks[[4]], hyper_b + as.matrix(0.5*t(par)%*%Klist[[4]]%*%par))
    Ks[[4]] <-Klist[[4]]/tau2
    
    
    par <- beta_out[iter, 43:53]  
    tau2_out[iter,5] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*ranks[[5]], hyper_b + as.matrix(0.5*t(par)%*%Klist[[5]]%*%par))
    Ks[[5]] <-Klist[[5]]/tau2
    
    par <- beta_out[iter, 54:64]  
    tau2_out[iter,6] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*ranks[[6]], hyper_b + as.matrix(0.5*t(par)%*%Klist[[6]]%*%par))
    Ks[[6]] <-Klist[[6]]/tau2
    
    par <- beta_out[iter, 65:75]  
    tau2_out[iter,7] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*ranks[[7]], hyper_b + as.matrix(0.5*t(par)%*%Klist[[7]]%*%par))
    Ks[[7]] <-Klist[[7]]/tau2
    
    # S <- as.matrix(bdiag(Ks[[1]], matrix(0,p0,p0), bdiag(Ks[2:(pnlin)])))
    
    S <- as.matrix(bdiag(Ks[[1]], Ks[[2]], Ks[[3]], Ks[[4]], matrix(0,p0,p0), Ks[[5]], Ks[[6]], Ks[[7]]))
...
#   bt <- param
#   exp_ident <- xx$ind_exp+1
#   bt[exp_ident] <- exp(bt[exp_ident])
#   sum(stats::dlogis(xx$X%*%bt, log=T)) + sum(log(xx$Xp%*%bt))
# }
