...
NUTS <- function(n_iter, xx, f, gr, ll, start, warmup = floor(n_iter/2),thin=1, burnin = floor(n_iter/2),
  ...
  ddet.mp <- function(x) {
    ...
    # rank <- rankMatrix((x))
    # sum(log(sigma[1:rank]))
    sum(log(sigma[sigma!=0]))
    # sum(log(sigma[1:96]))
  }
  # posterior_u <- function(u, par, rank, K, theta) (1 / exp(u))^rank * ginv(K)^(-0.5)
  
  # det(2pi Si)
  rank <- rankMatrix(Klist[[4]][[1]] + Klist[[4]][[2]])
  omega <- 0.05
  
  n_omega <- 11
  omega_grid <- seq(0.05, 0.95, length = n_omega)
  
  pre_Ks <- lapply(omega_grid, function(omega) Klist[[4]][[1]]*omega + (1 - omega) * Klist[[4]][[2]])
  K_determinants <- lapply(pre_Ks, function(K) ddet.mp(K))
  
  pre_ranks <- lapply(pre_Ks, rankMatrix)
  
  ranks <-rapply(Klist, rankMatrix, how="list")
  start_tau2_x <- 0.01
  start_tau2_y <- 0.01
  omega_start <- 0.1
  start_tau2 <- 0.0001
  ...
  posterior_u <- function(u, rank, par, K, theta){
    
    - (rank / 2) * u - t(par)%*%K%*%par / (2*exp(u)) + u/2 - exp(u)/(2*theta^2) 
  } 
  
  grad_u <- function(u, rank, par, K, theta){
    0.5 * (1 - rank + t(par)%*%K%*%par / exp(u) - exp(u) / theta^2)
  }
  
  hess_u <- function(u, par, K, theta){
    -0.5 *( t(par)%*%K%*%par / exp(u) - exp(u) / theta^2)
  }
  ...
  Kxy_start <- 1/start_tau2 * Klist[[4]][[1]] + 1/start_tau2 * Klist[[4]][[2]]
  # Smats <- vector("list", 2)
  # n_pen_grpss <- xx$npen
  ...
  tau2_out <- matrix(1, nrow=n_iter, ncol=4)
  ...
  for(iter in 1:n_iter){
    ...
    # update tau2 for npo effect
    par <- beta_out[iter,ind_npo]
    tau2_out[iter,1] <- tau2_npo <-  rinvgamma(1, hyper_a + 0.5*(ranks[[2]]), hyper_b + as.matrix(0.5*t(par)%*%Klist[[2]]%*%par))
    
    
    # update tau2 for random effect
    par <- beta_out[iter,ind_re]
    tau2_out[iter,2] <- tau2_re <-  rinvgamma(1, hyper_a + 0.5*(ranks[[3]]), hyper_b + as.matrix(0.5*t(par)%*%Klist[[3]]%*%par))
    
    
    # update tau2s for spatial effect
    par <- beta_out[iter,ind_spat]
    
    
    tau2_xy <- if(iter == 1) start_tau2 else tau2_out[iter - 1, 3]
    uc <- log(tau2_xy)
    
    # K_xy <- if(iter == 1)  (omega_start * Klist[[4]][[1]] + (1-omega_start) * Klist[[4]][[2]]) / start_tau2 else K_xy
    
    K_xy <- if(iter == 1)  Klist[[4]][[1]] +  Klist[[4]][[2]] else K_xy
    
    omega <- if(iter == 1) omega_start else omega
    
    tau2_out[iter,3] <- tau2_xy <-  rinvgamma(1, hyper_a + 0.5*rank, hyper_b + as.matrix(0.5*t(par)%*%(omega*Klist[[4]][[1]] +  (1-omega)*Klist[[4]][[2]])%*%par))
    
    # grad_uc <- grad_u(uc, rank, par, K_xy, theta)
    # 
    # hess_uc <- hess_u(uc, par, K_xy, theta)
    # sigma2_uc <- -1 / hess_uc
    # mu_uc <- sigma2_uc * grad_uc + uc
    # 
    # 
    # u_proposal <- rnorm(1, mu_uc, sqrt(sigma2_uc))
    # 
    # grad_up <- grad_u(u_proposal, rank, par,K_xy, theta)
    # hess_up <- hess_u(u_proposal, par, K_xy, theta)
    # 
    # sigma2_up <- -1 / hess_up
    # mu_up <- sigma2_up * grad_up + u_proposal
    # 
    ...
    # 
    # log_probab = posterior_u(u_proposal, par = par, rank = rank, K = K_xy, theta = theta) -
    #   posterior_u(u = uc, par = par, rank = rank, K = K_xy, theta = theta) + 
    #   dnorm(uc, mean = mu_up, sd = sqrt(sigma2_up), log = T) -
    #   dnorm(u_proposal, mean = mu_uc, sd = sqrt(sigma2_uc), log = T)
    # 
    # 
    # if (runif(1) < exp(log_probab)){
    #   tau2_out[iter, 3] = tau2_xy = exp(u_proposal)
    # }else{
    #   tau2_out[iter, 3] = tau2_xy
    # }


 omega_probs <- sapply(1:n_omega, function(w) 0.5*(K_determinants[[w]]) - 1/(2*tau2_xy) * (t(par)%*% (pre_Ks[[w]] %*% par)))  #- log(n_omega))
...
min_prob <- min(omega_probs)
    ...
    K_xy <- (omega * Klist[[4]][[1]] + (1-omega) * Klist[[4]][[2]])/ tau2_xy
    
    
    print(paste("omega: ", omega, "tau2: ", tau2_xy))
    
    S <- as.matrix(bdiag(K0, Klist[[2]]/tau2_npo, Klist[[3]]/tau2_re, K_xy))
