...
NUTS <- function(n_iter, xx, f, gr, ll, start, warmup = floor(n_iter/2),thin=1, burnin = floor(n_iter/2),
  ...
  for(iter in 1:n_iter){
    ...
    # # # 
    tau2_out[iter,1] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*(length(par)-2), hyper_b + as.matrix(0.5*t(par)%*%Klist[[1]]%*%par))
    Ks[[1]] <-Klist[[1]]/tau2
    # 
    # print(tau2)
    ...
    if(pnlin >0){
      ...
      tau2_out[iter,2] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*(length(par)-2), hyper_b + as.matrix(0.5*t(par)%*%Klist[[2]]%*%par))
      Ks[[2]] <-Klist[[2]]/tau2
      
      par <- beta_out[iter, 20:30]  
      tau2_out[iter,3] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*(length(par)-2), hyper_b + as.matrix(0.5*t(par)%*%Klist[[3]]%*%par))
      Ks[[3]] <-Klist[[3]]/tau2
      
      par <- beta_out[iter, 31:41]  
      tau2_out[iter,4] <- tau2 <-  rinvgamma(1, hyper_a + 0.5*(length(par)-2), hyper_b + as.matrix(0.5*t(par)%*%Klist[[4]]%*%par))
      Ks[[4]] <-Klist[[4]]/tau2
